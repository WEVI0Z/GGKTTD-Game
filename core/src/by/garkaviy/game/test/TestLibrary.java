...
public class TestLibrary {
    ...
    public static List<TestEntity> getTests() {
        String fileName = "firstTests.json";
        if (!Gdx.files.local("tests/" + fileName).exists()) {
            return null;
        }
        FileHandle file = Gdx.files.local("tests/" + fileName); // указываем путь к файлу
        String jsonData = file.readString(); // чтение JSON строки из файла

        Json json = new Json();
        return json.fromJson(TestCollection.class, jsonData).getTests(); // преобразование JSON строки в объект
    }
